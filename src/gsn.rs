...
impl TryFrom<Value> for AbsoluteIndex {
    ...
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        let int_val = value
            ...
            .or_else(|| value.as_str().and_then(|s| str::parse::<u32>(s).ok()));
        match int_val {
            Some(x) => Ok(AbsoluteIndex::Number(x)),
            None => {
                if value == "last" {
        ...
        map.into_iter().for_each(|(k, v)| {
            result.insert(
                ...
                if v.is_string() {
                    ...
                    serde_saphyr::to_string(&v).unwrap()
                },
...
mod test {
    ...
    fn serde_hor_invalid_index() -> Result<()> {
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("+asdf");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("");
        assert!(res.is_err());
        // 2**32 + 1
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("4294967297");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("+4294967297");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("-4294967297");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("-x");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("bslkdf");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("!absolute null");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("!absolute");
        assert!(res.is_err());
        let res: Result<HorizontalIndex, _> = serde_saphyr::from_str("null");
        assert!(res.is_err());
        Ok(())
    }
...
horizontalIndex:
  relative: -23
"#;
        let res: GsnDocument = serde_saphyr::from_str(goal)?;
    ...
    #[test]
    fn nodetype() -> Result<()> {
        let nt: GsnNodeType = serde_saphyr::from_str("Solution")?;
...
C1:
  ...
  nodeType: Solution
"#;
        let res: BTreeMap<String, GsnDocument> = serde_saphyr::from_str(gsn)?;
...
G1:
  ...
  acp:
    ...
    - G2
"#;
        assert!(serde_saphyr::from_str::<BTreeMap<String, GsnDocument>>(gsn).is_err());
    }
